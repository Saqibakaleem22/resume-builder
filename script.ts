...
const resumeAbout = document.getElementById("resumeAbout") as HTMLHeadingElement;
const resumeContact = document.getElementById("resumeContact") as HTMLHeadingElement;
...
function updateFormFromResume() {
    ...
    (document.getElementById("about") as HTMLInputElement).value = resumeAbout.textContent || '';
    ...
    (document.getElementById("location") as HTMLInputElement).value = resumeLocation.textContent?.replace('location: ', '') || '';
    ...
    (document.getElementById("language") as HTMLTextAreaElement).value = resumeLanguage.textContent || '';
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (name || about || email || phone || location || degree || education || workExperience || skills || language) {
        // Populate the resume preview if query params are present
        resumeName.textContent = name;
        resumeName.textContent = about;
        resumeEmail.textContent = `Email: ${email}`;
        resumePhone.textContent = `Phone: ${phone}`;
        resumeLocation.textContent = `location: ${location}`;
        resumeEducation.textContent = `${degree} from ${education}`;
